...
export function parseMultipart(buffer, contentType) {
    ...
    while (pos >= 0) {
        ...
        // 终止边界 `--boundary--`
        if (buffer.slice(start, start + 2).toString('utf-8') === '--')
            break;
...
function parsePartHeaders(headerBlock, data) {
    let name;
    let filename;
    let mimeType;
...
async function resolveSessionCwd(ctx, sessionId) {
    ...
    if (typeof liveCwd === 'string' && liveCwd.trim())
        return liveCwd;
 ...
 */
export function registerFileRoutes(ctx, router, config) {
    // ---- 目录列表预览 ----
    router.get('/sessions/:id/files', async (_req, res, params, query) => {
    ...
    // ---- 文件下载 / 内联预览 ----
    router.get('/sessions/:id/files/download', async (_req, res, params, query) => {
        const sessionId = params.id;
        const cwd = await resolveSessionCwd(ctx, sessionId);
        if (!cwd) {
            sendJson(res, 404, { ok: false, error: `Session '${sessionId}' not found or has no workspace cwd`, code: 'NOT_FOUND' });
        ...
        try {
            ...
            res.setHeader('Content-Disposition', `${inline ? 'inline' : 'attachment'}; filename="${asciiFallback}"; filename*=UTF-8''${encodeURIComponent(name)}`);
    ...
    // ---- 上传 ----
    router.post('/sessions/:id/files', async (_req, res, params, query, body) => {
        const sessionId = params.id;
        ...
        if (/^multipart\/form-data/i.test(contentType)) {
            ...
            const rawName = (query.filename || _req.headers['x-filename'] || '').trim();
    ...
    const stageDirOf = (cwd) => path.join(cwd, '.dsh-uploads');
    function stagePathsOf(cwd, uploadId) {
    ...
    async function readStageMeta(metaFile) {
        try {
    ...
    async function stageReceived(partFile) {
        try {
    ...
    router.post('/sessions/:id/files/resumable', async (_req, res, params, query) => {
        ...
        try {
            ...
            const meta = { name, size, mimeType, createdAt: Date.now() };
    ...
    router.get('/sessions/:id/files/resumable/:uploadId', async (_req, res, params) => {
        const cwd = await resolveSessionCwd(ctx, params.id);
    ...
    router.add('PUT', '/sessions/:id/files/resumable/:uploadId', async (_req, res, params, query, body) => {
        const cwd = await resolveSessionCwd(ctx, params.id);
        const paths = cwd ? stagePathsOf(cwd, params.uploadId) : undefined;
        if (!cwd || !paths) {
            sendJson(res, 400, { ok: false, error: '非法 uploadId', code: 'BAD_REQUEST' });
        ...
        const chunk = Buffer.isBuffer(body) ? body : Buffer.alloc(0);
        if (chunk.length === 0) {
    ...
    router.post('/sessions/:id/files/resumable/:uploadId/complete', async (_req, res, params) => {
        const cwd = await resolveSessionCwd(ctx, params.id);
        const paths = cwd ? stagePathsOf(cwd, params.uploadId) : undefined;
        if (!cwd || !paths) {
            sendJson(res, 400, { ok: false, error: '非法 uploadId', code: 'BAD_REQUEST' });
    ...
    router.delete('/sessions/:id/files/resumable/:uploadId', async (_req, res, params) => {
        const cwd = await resolveSessionCwd(ctx, params.id);
        const paths = cwd ? stagePathsOf(cwd, params.uploadId) : undefined;
        if (!cwd || !paths) {
            sendJson(res, 400, { ok: false, error: '非法 uploadId', code: 'BAD_REQUEST' });
