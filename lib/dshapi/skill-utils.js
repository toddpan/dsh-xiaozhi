...
const DEFAULT_CUSTOM_SKILL_DIRS = [];
/** 解析技能根目录（缺省 user-dsh）。project 需要 cwd；custom 取服务配置。 */
export function resolveSkillRoot(kind, cwd, customSkillDirs = DEFAULT_CUSTOM_SKILL_DIRS, dshHome = process.env.DSH_HOME || path.join(homedir(), '.dsh'), agentsHome = process.env.DSH_AGENTS_HOME || path.join(homedir(), '.agents'), bundledDir = process.env.DSH_BUNDLED_SKILL_DIR) {
    switch (kind || 'user-dsh') {
        ...
        case 'custom': {
            if (customSkillDirs.length === 0)
                return { ok: false, error: '该服务未配置自定义技能目录 (customSkillDirs)' };
            return { ok: true, root: { kind: 'custom', path: customSkillDirs[0] } };
        }
        case 'project': {
            if (!cwd)
                return { ok: false, error: 'root=project 需要提供 cwd 参数' };
        ...
        case 'bundled': {
            if (!bundledDir)
                return { ok: false, error: '该服务未内置 bundle 技能目录 (DSH_BUNDLED_SKILL_DIR)' };
...
export function parseSkillFile(raw) {
    ...
    if (!name || !description || !isSkillName(name))
        return undefined;
    return {
        name,
        description,
...
export function parseYamlSubset(text) {
    const result = { modelInvocable: true, userInvocable: true };
    ...
    if (typeof result['disable-model-invocation'] === 'boolean') {
        result.modelInvocable = !result['disable-model-invocation'];
    }
...
function scalarValue(raw) {
    ...
    if (v === 'false')
        return false;
    const n = Number(v);
    if (v !== '' && !Number.isNaN(n) && /^-?\d+(\.\d+)?$/.test(v))
        return n;
    // inline comment 去除
...
export async function extractZip(buffer, destDir) {
    try {
        ...
        if (!eocd)
            return { ok: false, error: '无法定位 ZIP 中央目录 (EOCD)' };
        ...
        for (let i = 0; i < count; i += 1) {
            if (buffer.readUInt32LE(pos) !== 0x02014b50)
                return { ok: false, error: '中央目录条目签名错误' };
            ...
            if (buffer.readUInt32LE(localOffset) !== 0x04034b50)
                return { ok: false, error: `local header 签名错误: ${fname}` };
...
function safeZipName(name) {
    ...
    for (const seg of segments) {
        if (seg === '..' || seg === '')
            return undefined;
        if (seg.endsWith(':') || /^[A-Za-z]:/.test(seg))
            return undefined;
    }
    return normalized;
}
/** tar / tgz / tar.gz 走系统 tar（strip 顶层包装目录）。返回是否成功。 */
export async function extractTar(buffer, destDir, strip) {
