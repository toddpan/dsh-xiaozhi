 ...
 */
function snapshotLiveEvents(liveSession, fallback) {
...
export function foldSessionStats(events) {
    ...
    const hasMessageUsage = (events || []).some((ev) => ev?.type === 'assistant/message' && ev?.data?.usage && typeof ev.data.usage === 'object');
        ...
        switch (ev?.type) {
            ...
            case 'assistant/message': {
                ...
                if (usage && typeof usage === 'object') {
                    if (finiteNonNegative(usage.inputTokens))
                        out.usage.inputTokens += usage.inputTokens;
                    if (finiteNonNegative(usage.cacheReadTokens))
                        out.usage.cacheReadTokens += usage.cacheReadTokens;
                    if (finiteNonNegative(usage.cacheWriteTokens))
                        out.usage.cacheWriteTokens += usage.cacheWriteTokens;
                    if (finiteNonNegative(usage.outputTokens))
            ...
            case 'step/end': {
                out.steps += 1;
                if (finiteNonNegative(ev.data?.turn))
                    turnsSeen.add(ev.data.turn);
                closeStep();
                break;
            }
            case 'tool/call': {
                const callId = ev.data?.callId;
                if (typeof callId === 'string' && callId && t !== null)
                    pendingCalls.set(callId, t);
                break;
            }
            case 'tool/result': {
                const callId = ev.data?.message?.content?.find?.((c) => c?.type === 'tool-result')?.toolCallId
                    ?? ev.data?.callId;
            ...
            case 'usage': {
                ...
                if (!hasMessageUsage) {
                    ...
                    if (u && typeof u === 'object') {
                        if (finiteNonNegative(u.inputTokens))
                            out.usage.inputTokens += u.inputTokens;
                        if (finiteNonNegative(u.cacheReadTokens))
                            out.usage.cacheReadTokens += u.cacheReadTokens;
                        if (finiteNonNegative(u.cacheWriteTokens))
                            out.usage.cacheWriteTokens += u.cacheWriteTokens;
                        if (finiteNonNegative(u.outputTokens))
 ...
 */
export function foldSessionTodos(events, now = Date.now()) {
    const out = {
    ...
    for (const ev of events || []) {
        const t = eventTime(ev);
        if (t !== null)
            out.lastEventAt = out.lastEventAt === null ? t : Math.max(out.lastEventAt, t);
        switch (ev?.type) {
            case 'turn/start': {
...
function liveRunningState(ctx, sessionId) {
...
export function registerSessionRoutes(ctx, router, config) {
    // 1. 查询会话列表
    router.get('/sessions', async (_req, res, _params, query) => {
        try {
            const sessionController = ctx.get('sessionController');
            ...
            if (query.search && query.search.trim()) {
                ...
                rawList = rawList.filter((s) => (s.title && s.title.toLowerCase().includes(kw)) ||
            ...
            if (query.workspaceId && workspaceRegistry) {
                ...
                if (ws) {
                    ...
                    rawList = rawList.filter((s) => validIds.has(String(s.sessionId || s.id)));
                }
                else {
                    rawList = [];
                }
    ...
    // 2. 查询单个会话详情
    router.get('/sessions/:id', async (_req, res, params) => {
        try {
            const sessionController = ctx.get('sessionController');
            const workspaceRegistry = ctx.get('workspaceRegistry');
    ...
    // 3. 创建会话 (添加)
    router.post('/sessions', async (_req, res, _params, _query, body) => {
        try {
            const sessionController = ctx.get('sessionController');
            const workspaceRegistry = ctx.get('workspaceRegistry');
            ...
            if (body.provider && body.model && sessionController?.selectModel) {
                try {
                    ...
                    const previousDefault = defaultModelService && typeof defaultModelService.currentSelection === 'function'
    ...
    // 4. 修改会话 (修改标题 / 修改模型)
    router.put('/sessions/:id', async (_req, res, params, _query, body) => {
        try {
            const sessionController = ctx.get('sessionController');
            const sessionId = params.id;
            if (!body) {
    ...
    // 5. 删除/归档会话 (删除)
    router.delete('/sessions/:id', async (_req, res, params) => {
        try {
            const workspaceRegistry = ctx.get('workspaceRegistry');
            const sessionId = params.id;
    ...
    // 6. 获取会话历史记录 (分页)
    router.get('/sessions/:id/history', async (_req, res, params, query) => {
        try {
            const sessionController = ctx.get('sessionController');
            const sessionId = params.id;
            ...
            if (beforeSeq !== undefined) {
                events = events.filter((e) => e.seq < beforeSeq);
            }
            // 解析提取对话消息
            const allMessages = [];
            for (const ev of events) {
    ...
    router.get('/sessions/:id/stats', async (_req, res, params) => {
        try {
            const sessionId = params.id;
    ...
    router.get('/sessions/:id/todos', async (_req, res, params) => {
        try {
            const sessionId = params.id;
            const events = await loadSessionEvents(ctx, sessionId);
            const now = Date.now();
    ...
    router.get('/sessions/:id/skills', async (_req, res, params, query) => {
        try {
            const sessionId = params.id;
            ...
            if ('error' in resolved)
                return sendJson(res, 400, { ok: false, error: resolved.error, code: 'BAD_REQUEST' });
    ...
    // 7. 取消当前轮次执行
    router.post('/sessions/:id/cancel', async (_req, res, params) => {
        try {
            const sessionController = ctx.get('sessionController');
            const sessionId = params.id;
