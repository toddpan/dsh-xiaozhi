...
export function registerModelRoutes(ctx, router) {
    // 1. 获取所有可用模型列表
    ...
    // 3. 修改全局默认模型设置
    router.put('/models/default', async (_req, res, _params, _query, body) => {
        try {
            const defaultModelService = ctx.get('agentDefaultModel');
    ...
    // 7. 更新指定命名空间的系统设置
    router.patch('/settings/:namespace', async (_req, res, params, _query, body) => {
        try {
            const settingsController = ctx.get('settingsController');
