...
import { discoverSkills, extractTar, extractZip, isSkillName, parseSkillFile, parseYamlSubset, resolveSkillRoot, serializeFrontmatter, } from './skill-utils.js';
const execFileAsync = promisify(execFile);
export function registerSkillRoutes(ctx, router, config) {
    const maxBytes = config.maxUploadBytes || 2 * 1024 * 1024 * 1024;
    const customDirs = config.customSkillDirs || [];
    const rootOf = (kind, cwd) => {
    ...
    // ---- 列表 ----
    router.get('/skills', async (_req, res, _params, query) => {
        const root = rootOf(query.root, query.cwd);
        if ('error' in root)
            return sendJson(res, 400, { ok: false, error: root.error, code: 'BAD_REQUEST' });
        try {
    ...
    // ---- 单技能详情（含全文） ----
    router.get('/skills/:name', async (_req, res, params, query) => {
        const root = rootOf(query.root, query.cwd);
        if ('error' in root)
            return sendJson(res, 400, { ok: false, error: root.error, code: 'BAD_REQUEST' });
        const name = params.name;
        if (!isSkillName(name))
            return sendJson(res, 400, { ok: false, error: `非法技能名: ${name}`, code: 'BAD_REQUEST' });
        const skillPath = skillPathFor(name, root.path);
        if (!existsSync(skillPath))
            return sendJson(res, 404, { ok: false, error: `技能 ${name} 不存在`, code: 'NOT_FOUND' });
        try {
            const raw = await readFile(skillPath, 'utf-8');
            const parsed = parseSkillFile(raw);
            if (!parsed)
                return sendJson(res, 400, { ok: false, error: `技能文件 ${skillPath} 解析失败`, code: 'BAD_REQUEST' });
    ...
    // ---- SKILL.md 原始全文（下载/预览） ----
    router.get('/skills/:name/body', async (_req, res, params, query) => {
        const root = rootOf(query.root, query.cwd);
        if ('error' in root)
            return sendJson(res, 400, { ok: false, error: root.error, code: 'BAD_REQUEST' });
        const name = params.name;
    ...
    // ---- 整个技能目录归档 (.tgz) ----
    router.get('/skills/:name/archive', async (_req, res, params, query) => {
        const root = rootOf(query.root, query.cwd);
        if ('error' in root)
            return sendJson(res, 400, { ok: false, error: root.error, code: 'BAD_REQUEST' });
        const name = params.name;
    ...
    router.post('/skills', async (_req, res, _params, query, body) => {
        const contentType = String(_req.headers['content-type'] || '');
        const buffer = Buffer.isBuffer(body) ? body : Buffer.alloc(0);
        if (buffer.length === 0)
            return sendJson(res, 400, { ok: false, error: '请求体为空：请以 multipart/压缩包或 JSON 上传技能', code: 'BAD_REQUEST' });
        if (buffer.length > maxBytes)
            return sendJson(res, 413, { ok: false, error: `Payload too large (> ${maxBytes} bytes)`, code: 'PAYLOAD_TOO_LARGE' });
        // multipart → 压缩包上传
        if (/^multipart\/form-data/i.test(contentType)) {
            ...
            for (const p of parts) {
                if (p.filename !== undefined && p.data.length > 0)
                    file = p;
                else if (p.name !== undefined)
                    fields[p.name] = p.data.toString('utf-8');
            }
            if (!file)
                return sendJson(res, 400, { ok: false, error: 'multipart 中未找到技能压缩包文件字段', code: 'BAD_REQUEST' });
            ...
            try {
                ...
                if (!result.ok)
                    return sendJson(res, 400, { ok: false, error: result.error, code: 'BAD_REQUEST' });
                invalidateSkills();
        ...
        if (!isSkillName(name))
            return sendJson(res, 400, { ok: false, error: `非法技能名: ${name || '(空)'}`, code: 'BAD_REQUEST' });
        const description = String(json.description || '').trim();
        if (!description)
            return sendJson(res, 400, { ok: false, error: 'description 不能为空', code: 'BAD_REQUEST' });
        try {
    ...
    // ---- 更新 ----
    router.put('/skills/:name', async (_req, res, params, query, body) => {
        const root = rootOf(query.root, query.cwd);
        if ('error' in root)
            return sendJson(res, 400, { ok: false, error: root.error, code: 'BAD_REQUEST' });
        const name = params.name;
        ...
        try {
            ...
            if (body?.description !== undefined)
                fm.description = String(body.description).trim();
            if (body?.whenToUse !== undefined)
                fm.whenToUse = String(body.whenToUse).trim();
    ...
    // ---- 删除 ----
    router.delete('/skills/:name', async (_req, res, params, query) => {
        const root = rootOf(query.root, query.cwd);
        if ('error' in root)
            return sendJson(res, 400, { ok: false, error: root.error, code: 'BAD_REQUEST' });
        const name = params.name;
...
function skillPathFor(name, rootPath) {
 ...
 */
async function installSkillArchive(rootPath, data, filename, explicitName) {
    ...
    try {
        ...
        if (!located) {
            ...
            if (topDirs.length === 1) {
                ...
                if (inner)
                    return finalizeSkill(inner, tmpDir, rootPath, explicitName);
            }
            await rm(tmpDir, { recursive: true, force: true }).catch(() => { });
...
async function locateSkillInTree(dir) {
    ...
    for (const f of (await readdir(dir)).filter((x) => x.endsWith('.md'))) {
        ...
        if (parsed)
            return { dir, skillPath: p, name: parsed.name, flat: true };
    }
    return undefined;
}
/** 将已解压的技能内容落盘为 <root>/<name>/（目录形式，统一含 SKILL.md）。 */
async function finalizeSkill(located, tmpDir, rootPath, explicitName) {
    ...
    if (!name || !isSkillName(name))
        return { ok: false, error: `无法确定技能名: ${name || '(空)'}` };
    ...
    await rm(tmpDir, { recursive: true, force: true }).catch(() => { });
    if (!existsSync(finalSkill))
        return { ok: false, error: '技能目录缺少 SKILL.md' };
...
async function createSkillFromJson(rootPath, json) {
    const name = String(json.name).trim();
    const targetDir = path.join(rootPath, name);
    await mkdir(targetDir, { recursive: true });
    const fm = {
