...
import { InvokeError } from './dispatcher.js';
...
export class CapabilityError extends Error {
    capability;
    status;
    constructor(message, capability, status) {
        super(message);
...
export class CapabilityRuntime {
    ...
    refIndex() {
        ...
        const promise = (async () => {
            ...
            const rows = (value) => (Array.isArray(value) ? value : []).map((raw) => ({ id: String(raw?.id ?? ''), title: String(raw?.title ?? '') })).filter(row => row.id !== '');
            return { sessions: rows(sessions), workspaces: rows(workspaces) };
        })();
        this.refCache = { at: Date.now(), promise };
        return promise;
    }
    async resolveRef(kind, raw) {
        ...
        if (rows.some(row => row.id === text))
            return text;
        const lowered = text.toLowerCase();
        const matches = rows.filter(row => row.id.toLowerCase().includes(lowered) ||
        ...
        if (matches.length > 1) {
            ...
            throw new CapabilityError(`${noun} "${clip(text, 40)}" 匹配到 ${matches.length} 个（${shown}${matches.length > 5 ? ' 等' : ''}），` +
                '请改用列表里更完整的短 id 或更精确的标题', undefined, 400);
        }
        throw new CapabilityError(`找不到${noun} "${clip(text, 40)}"。请先用列表动作查看现有的${noun}短 id。`, undefined, 404);
    }
    /** Expand ids for the capability's path/body/query before dispatch. */
    async resolveArgs(spec, args) {
        const resolved = { ...args };
    ...
    async run(id, args) {
        ...
        if (!spec)
            throw new CapabilityError(`unknown capability "${id}"`, id);
        const config = this.deps.config();
        if (spec.write && !config.allowWriteTools) {
        ...
        try {
            ...
            throw new CapabilityError(err?.message ?? String(err), id);
        }
    }
    // ---------------------------------------------------------------- REST
    async dispatchRest(spec, args, config) {
        const path = spec.path.replace(/:([a-zA-Z0-9_]+)/g, (_match, name) => {
        ...
        if (spec.body === 'json') {
            ...
            if (content === '')
                throw new CapabilityError('缺少参数 "content"', spec.id);
            body = Buffer.from(content, encoding === 'utf8' ? 'utf8' : 'base64');
            if (!query.filename && typeof args.filename === 'string')
                query.filename = args.filename;
            if (Buffer.isBuffer(body) && body.length > config.maxUploadBytes) {
                throw new CapabilityError(`文件超过大小上限（${body.length} > ${config.maxUploadBytes} 字节）`, spec.id);
            }
        }
        ...
        if (spec.id === 'files.upload') {
            ...
            if (typeof query.filename === 'string')
                headers['x-filename'] = query.filename;
        }
        ...
        if (spec.id === 'files.download' || spec.id === 'docs.info')
     ...
     */
    async sessionEvents(args, config) {
        ...
        await new Promise(resolve => {
            let settled = false;
            const dispose = this.deps.ctx.on('session/event', (session, event) => {
...
function timeoutFor(spec, args, config) {
...
function requireString(value, name) {
    ...
    if (text === '')
        throw new CapabilityError(`缺少参数 "${name}"`);
    return text;
}
