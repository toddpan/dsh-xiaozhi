 ...
 */
import { RPC_ERROR, clip, initializeResult, parseEnvelope, readToolCall, rpcError, rpcNotification, rpcResult, toolsListResult, } from './protocol.js';
        ...
        switch (envelope.kind) {
            ...
            case 'notification':
                ...
                if (envelope.method === 'notifications/initialized')
                    ...
                    this.deps.log(`notification ${envelope.method} ignored`);
                return;
            case 'request':
                break;
            default:
                    ...
                    try {
                        ...
                        this.send(rpcResult(id, {
                            content: [
                                {
                                    type: 'text',
                                    text: `执行工具 ${call.name} 时发生内部错误：${clip(err?.message ?? String(err), 300)}`,
                                },
                            ],
                            isError: true,
                        }));
