 ...
 */
import { Config as ConfigShape, clearOverrides, effectiveEndpoints, mergeMaskedEndpoints, readOverrides, redactConfig, resolveConfig, resolveSettingsFile, writeOverrides, } from './config.js';
import { LocalInvoker } from './dispatcher.js';
import { createDshApiRouter, sendJson } from './dshapi/service.js';
import { CAPABILITIES, CapabilityRuntime, TOOL_GROUPS, TOOL_GROUP_LABELS, pluginVersion, } from './capabilities.js';
import { ToolRunner } from './tools.js';
import { McpSession } from './mcp-server.js';
import { EndpointTransport, ServerTransport, TransportStatus, maskEndpoint, } from './transports.js';
...
export function apply(ctx, rowConfig) {
    ...
    let liveStatus;
    const requireLiveStatus = () => {
        // Assigned synchronously during boot, before any request can be dispatched.
        if (!liveStatus)
            throw new Error('runtime status provider is not ready');
        return liveStatus();
    };
    const boot = () => {
        ...
        // ---- transport ------------------------------------------------------
        const createSession = (connection) => new McpSession(connection, {
        ...
        if (!config.enabled) {
            ...
            serverTransport = new ServerTransport({
                ...
                config: () => resolved,
                status,
                createSession: (connection) => createSession(connection),
                log: message => log.push(message),
            });
        }
        else {
        ...
        const dispatchWith = (router, base, label) => {
            return async (req, res) => {
                try {
                    ...
                    log.push(`${label} dispatch failed: ${err?.message ?? String(err)}`);
                    if (!res.headersSent)
        ...
        liveStatus = () => adminStatus(endpointDevices, serverTransport, status, resolved, runner, capabilities, apiBase, adminBase, rowConfig, log);
        ...
        return {
            dispose() {
                try {
                    ...
                    log.push(`transport stop failed: ${err?.message ?? String(err)}`);
                }
 ...
 */
export function swapRuntime(previous, boot, log) {
    previous?.dispose();
    try {
        return boot();
    }
    catch (err) {
        log(`runtime restart failed: ${err?.message ?? String(err)}`);
        throw err;
    }
}
/**
 ...
 */
function aggregateSnapshot(devices, server, status, resolved) {
    if (server)
        return server.snapshot();
    if (devices.length > 0) {
        const snaps = devices.map(device => device.transport.snapshot());
        const rank = { ready: 0, connecting: 1, error: 2, idle: 3, disabled: 4 };
 ...
 */
function hostConnectionRejection(ctx, request) {
...
async function adminStatus(endpointDevices, serverTransport, status, resolved, runner, capabilities, apiBase, adminBase, rowConfig, log) {
    ...
    return {
        ...
        devices: resolved.enabled && resolved.mode === 'endpoint'
        ...
        groups: TOOL_GROUPS.map(group => ({
            ...
            exposed: capabilities.available().some(spec => spec.group === group),
        })),
 ...
 */
function collectWarnings(resolved, runner) {
    const warnings = [];
    if (!resolved.enabled) {
    ...
    if (resolved.mode === 'endpoint') {
        ...
        for (const device of devices) {
            ...
            if (problem)
                warnings.push(`设备「${device.name || device.id}」：${problem}`);
        }
    }
    else {
 ...
 */
async function testConnection(resolved, endpointDevices, serverTransport, target, log) {
    if (!resolved.enabled) {
    ...
    const probeOne = async (url, headers, label) => {
    ...
    if (target.id) {
        ...
        if (!device)
            return { ok: false, message: '找不到该设备，请先保存配置后再测试。' };
    ...
    const results = await Promise.all(devices.map(device => probeOne(device.url, { ...resolved.endpointHeaders, ...(device.headers ?? {}) }, device.name || device.id)));
