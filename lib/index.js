 ...
 */
import { Config as ConfigShape, clearOverrides, readOverrides, redactConfig, resolveConfig, resolveSettingsFile, writeOverrides, } from './config.js';
...
export function apply(ctx, rowConfig) {
    ...
    const boot = () => {
        ...
        const adminRouter = createAdminRouter({
            ...
            reconnect: async () => {
                if (transport instanceof EndpointTransport)
                    transport.reconnect();
                else
...
function transportSnapshot(status, transport, resolved) {
    if (transport)
...
async function adminStatus(status, transport, resolved, runner, capabilities, apiBase, adminBase, rowConfig, log) {
...
async function testConnection(resolved, transport, log) {
    if (!resolved.enabled) {
