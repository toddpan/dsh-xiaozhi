...
import { SECRET_MASK, isMaskedEndpoint, type Config } from './config.js'
import { HttpRouter, sendJson, type RouteHandler } from './dshapi/router.js'
...
export function createAdminRouter(deps: AdminApiDeps): HttpRouter {
  const router = new HttpRouter({ cors: false, apiKey: '' })

  const guard = (mutating: boolean, handler: Handler): Handler => {
  ...
  router.patch(
    '/config',
    guard(true, async (req, res, _params, _query, body) => {
      ...
      for (const key of ['apiKey', 'serverToken'] as const) {
        if (patch[key] === SECRET_MASK) delete patch[key]
      }
      if (isMaskedEndpoint(patch.endpointUrl)) delete patch.endpointUrl
      if (patch.endpointHeaders) {
        patch.endpointHeaders = Object.fromEntries(
          Object.entries(patch.endpointHeaders).filter(([, value]) => value !== SECRET_MASK),
        )
      }
