...
function relativeTime(at: unknown): string {
  const ms = typeof at === 'number' && Number.isFinite(at) ? at : undefined
  if (ms === undefined) return '未知时间'
...
function renderRows(rows: string[], limit: number, noun: string): string {
  if (rows.length === 0) return `没有${noun}。`
  const head = rows.slice(0, limit)
  const rest = rows.length - head.length
  return rest > 0 ? `${head.join('\n')}\n（共 ${rows.length} 条，仅显示前 ${limit} 条）` : head.join('\n')
}

function renderSessionRow(raw: unknown): string {
...
function renderWorkspaceRow(raw: unknown): string {
  const w = raw as Record<string, unknown>
  const title = clip(String(w.title ?? w.name ?? '未命名工作区'), 40)
  const count = Array.isArray(w.sessionIds) ? (w.sessionIds as unknown[]).length : 0
  return `- ${title} [${shortId(w.id)}] ${count} 个会话 · ${clip(String(w.path ?? ''), 60)}`
}

function renderTodoRow(raw: unknown): string {
  const t = raw as Record<string, unknown>
  const mark = t.status === 'completed' ? '✔' : t.status === 'in_progress' ? '▶' : '·'
  return `${mark} ${clip(String(t.content ?? ''), 50)}`
}

function msToText(ms: unknown): string {
...
const ACTION = (values: string[], description: string) => ({
  type: 'string',
  enum: values,
  description,
})
...
const GROUPED_TOOL_SPECS: readonly GroupedToolSpec[] = [
  {
    name: 'dsh_status',
    description:
      '查询 DSH（DeepSeek Harness）的运行状态：是否在线、监听端口、工作区数量、可用模型，以及与小智的连接状态。用户问“DSH 在运行吗”“小智连上 DSH 了吗”“有哪些模型”时使用。',
    properties: {},
    required: [],
    write: false,
    groups: ['system'],
    ...
    async run(_args, runtime) {
      const status = (await runtime.run('system.status', {})) as Record<string, any>
      const mcp = (status.mcp ?? {}) as Record<string, any>
      const lines = [
      ...
      if (mcp.lastError) lines.push(`最近错误：${clip(String(mcp.lastError), 120)}`)
      return lines.join('\n')
    },
  },
  {
    ...
    async run(args, runtime, config) {
      ...
      switch (action) {
        ...
        case 'get': {
          const ws = (await runtime.run('workspaces.get', { id: args.id })) as Record<string, any>
          return [
        ...
        case 'create': {
          const ws = (await runtime.run('workspaces.create', { path: args.path, title: args.title })) as Record<string, any>
          return `已创建工作区「${clip(String(ws.title ?? ''), 40)}」，ID ${ws.id}。`
        }
        case 'rename': {
          const ws = (await runtime.run('workspaces.update', { id: args.id, title: args.title })) as Record<string, any>
  ...
  },
  {
    ...
    async run(args, runtime, config) {
      ...
      switch (action) {
        ...
        case 'get': {
          const s = (await runtime.run('sessions.get', { id: args.id })) as Record<string, any>
          return [
  ...
  },
  {
    ...
    description:
      '查看某个 DSH 会话的运行进度：当前轮次/步骤、耗时、首 token 与吞吐、token 消耗，以及任务清单完成情况。用户问“那个会话跑到哪儿了”“任务做完了吗”时使用。',
    ...
    async run(args, runtime, config) {
      ...
      const lines: string[] = []
      const todoData = (todos ?? {}) as Record<string, any>
      lines.push(
        `运行状态：${todoData.running ? `执行中，已运行 ${msToText(todoData.elapsedMs)}` : '空闲'}。`,
      )
      const counts = (todoData.counts ?? {}) as Record<string, number>
      ...
      if (s && (s.turns !== undefined || s.steps !== undefined)) {
        lines.push(
          `统计：${s.turns ?? 0} 轮 / ${s.steps ?? 0} 步，模型耗时 ${msToText(s.llmMs)}，工具耗时 ${msToText(s.toolMs)}` +
            (s.ttftSteps ? `，首 token 平均 ${msToText(s.ttftMs / s.ttftSteps)}` : ''),
        )
        const usage = (s.usage ?? {}) as Record<string, number>
        if (usage.outputTokens !== undefined) {
          lines.push(
            `token：输入 ${usage.inputTokens ?? 0}，输出 ${usage.outputTokens ?? 0}，缓存命中 ${usage.cacheReadTokens ?? 0}`,
          )
        }
      }
      return lines.join('\n')
  ...
  },
  {
    ...
    async run(args, runtime, config) {
      ...
      if (prompt.trim() === '') throw new CapabilityError('prompt 不能为空')
      const wait = args.wait === undefined ? true : Boolean(args.wait)
      const timeoutMs = Math.min(
        30 * 60_000,
        Math.max(5_000, Math.round(Number(args.timeoutSeconds ?? config.promptTimeoutMs / 1000) * 1000)),
      )
      ...
      if (!wait) {
        ...
        await runtime
          ...
          .catch(() => undefined)
        const session = (await runtime.run('sessions.get', { id }).catch(() => undefined)) as Record<string, any> | undefined
  ...
  },
  {
    ...
    async run(args, runtime) {
      ...
      if (action === 'list') {
        const data = (await runtime.run('sessions.questions', { id })) as Record<string, any>
        const batches = asArray(data?.batches)
        if (batches.length === 0) return '该会话当前没有等待回答的问题。'
        const lines: string[] = [`共有 ${batches.length} 批待回答的问题：`]
        for (const raw of batches) {
          const batch = raw as Record<string, any>
          ...
          for (const q of asArray(batch.questions)) {
            ...
            const options = asArray(question.options)
              ...
              .filter(Boolean)
            lines.push(
              `- ${clip(String(question.question ?? ''), 120)}（id ${shortId(question.id)}）` +
      ...
      let answers: { id: string; selected: string[]; custom?: string }[] | undefined
      if (typeof args.answers === 'string' && args.answers.trim() !== '') {
        try {
          const parsed = JSON.parse(args.answers)
          if (!Array.isArray(parsed)) throw new Error('answers 必须是 JSON 数组')
          ...
          throw new CapabilityError(`answers 解析失败：${(err as Error).message}`)
        }
      }

      if (!answers) {
        const pending = (await runtime.run('sessions.questions', { id })) as Record<string, any>
        const batch = (asArray(pending?.batches)[0] ?? undefined) as Record<string, any> | undefined
        if (!batch) throw new CapabilityError('该会话当前没有挂起的问题')
  ...
  },
  {
    ...
    async run(args, runtime) {
      ...
      switch (action) {
        case 'list': {
          const data = (await runtime.run('files.list', { id, path: args.path })) as Record<string, any>
        ...
        case 'upload': {
          ...
          const saved = asArray(result?.files ?? result)
            .map(raw => String((raw as Record<string, any>)?.name ?? (raw as Record<string, any>)?.path ?? ''))
            .filter(Boolean)
  ...
  },
  {
    ...
    async run(args, runtime, config) {
      const data = (await runtime.run('sessions.skills', { id: args.id, search: args.search })) as Record<string, any>
      const skills = asArray(data?.skills ?? data)
      if (skills.length === 0) return '没有可用技能。'
  ...
  },
  {
    ...
    async run(args, runtime) {
      const data = (await runtime.run('sessions.events', { id: args.id, seconds: args.seconds })) as Record<string, any>
      const lines: string[] = [`${data.windowSeconds} 秒内共 ${data.totalEvents} 个事件。`]
      const byType = (data.byType ?? {}) as Record<string, number>
      const top = Object.entries(byType)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 6)
        .map(([type, n]) => `${type}×${n}`)
      if (top.length > 0) lines.push(`事件类型：${top.join('，')}`)
  ...
  },
  {
    name: 'dsh_chat',
    description:
      '用一次性问题直接问 DSH（OpenAI 兼容的一次性对话，不新建可见会话，也可用 sessionId 复用已有会话）。适合“问 DSH 一个问题”而不用挑会话。',
    properties: {
  ...
  },
  {
    name: 'dsh_models',
    description:
      '查看或切换 DSH 使用的模型。action=list 列出可用模型；default 查看当前默认模型；set_default 切换默认模型（需要 provider 与 model）。',
    properties: {
    ...
    async run(args, runtime, config) {
      const action = String(args.action ?? '').toLowerCase()
      if (action === 'list') {
        const data = (await runtime.run('models.list', {})) as Record<string, any>
      ...
      if (action === 'default') {
        const def = (await runtime.run('models.default', {})) as Record<string, any>
  ...
  },
  {
    ...
    async run(_args, runtime, config) {
      const data = (await runtime.run('models.presets', {})) as Record<string, any>
      const presets = asArray(data?.presets ?? data)
      if (presets.length === 0) return '没有可用的 Agent Preset。'
  ...
  },
  {
    name: 'dsh_settings',
    description:
      '读取或修改 DSH 系统设置。action=get 列出所有设置命名空间；action=patch 更新指定命名空间（namespace 必填，patch 是 JSON 对象字符串）。',
    properties: {
    ...
    async run(args, runtime, config) {
      const action = String(args.action ?? '').toLowerCase()
      if (action === 'get') {
        const data = (await runtime.run('settings.get', {})) as Record<string, any>
        const namespaces = asArray(data?.namespaces)
        if (namespaces.length === 0) return '没有可读取的设置命名空间。'
        const lines = namespaces.map(raw => {
          const ns = raw as Record<string, any>
          return `- ${ns.ns}`
        })
      ...
      if (action === 'patch') {
        ...
        if (namespace === '') throw new CapabilityError('namespace 必填')
        let patch: unknown
        try {
          patch = typeof args.patch === 'string' ? JSON.parse(args.patch) : args.patch
        } catch (err) {
          throw new CapabilityError(`patch 解析失败：${(err as Error).message}`)
        }
  ...
  },
  {
    name: 'dsh_docs',
    description:
      '获取 DSH 内置 API 文档与 OpenAPI 规范的访问地址，以及本插件当前暴露给小智的全部能力清单。',
    properties: {},
    required: [],
    write: false,
    groups: ['docs'],
    ...
    async run(_args, runtime) {
      const info = (await runtime.run('docs.info', {})) as Record<string, any>
      const lines = [
        `交互式文档：${info.docsUrl}`,
        `OpenAPI 规范：${info.openApiUrl}`,
        `共 ${info.totalCapabilities} 项能力，分为 ${(info.groups ?? []).length} 组。`,
      ]
      const spec = (await runtime.run('docs.openapi', {}).catch(() => undefined)) as
        | Record<string, any>
        | undefined
      if (spec?.openapi) {
...
const FLAT_PARAM_DOCS: Record<string, Record<string, FlatParam>> = {
  'workspaces.create': {
...
function defaultFlatParam(name: string, spec: CapabilitySpec): FlatParam {
...
function flatToolName(spec: CapabilitySpec): string {
  return canonicalToolName(`dsh_${spec.id.replace(/\./g, '_')}`)
}

function buildFlatSpec(spec: CapabilitySpec): DshToolDefinition {
  ...
  for (const name of names) {
    if (spec.id === 'settings.patch' && name === 'patch') continue // handled below
    const doc = docs[name] ?? defaultFlatParam(name, spec)
    const schema: Record<string, unknown> = { type: doc.type, description: doc.description }
    if (doc.enum) schema.enum = doc.enum
    if (doc.items) schema.items = doc.items
    properties[name] = schema
    if (doc.required || spec.pathParams.includes(name)) required.push(name)
  }
...
export function buildTools(config: ResolvedConfig): BuiltTools {
  const groupDisabled = (id: string): boolean => {
    const cap = getCapability(id)
    return cap === undefined || config.disabledGroups.includes(cap.group)
  }
  const writeDisabled = (id: string): boolean => getCapability(id)?.write === true && !config.allowWriteTools
  ...
  const groupedRunners = new Map<string, GroupedToolSpec>()
  for (const spec of GROUPED_TOOL_SPECS) {
    ...
    const hasDisabledWrites =
      !config.allowWriteTools && spec.capabilities.some(id => getCapability(id)?.write === true)
...
export class ToolRunner {
  ...
  async call(name: string, args: Record<string, unknown>): Promise<ToolCallResult> {
    ...
    const tool =
      this.built.tools.find(item => item.name === requested) ??
    ...
    if (!tool) {
      return toolCallResult(
        `没有名为 ${requested} 的工具。可用工具：${this.built.tools.map(t => t.name).join('、')}`,
        true,
      )
    }
  ...
  private async runTool(tool: DshToolDefinition, args: Record<string, unknown>): Promise<unknown> {
    ...
    if (this.built.mode === 'flat') {
      ...
      if (!spec) throw new CapabilityError(`工具 ${tool.name} 未绑定能力`)
    ...
    if (!runner) throw new CapabilityError(`工具 ${tool.name} 未注册执行器`)
    const value = await runner.run(args, this.deps.runtime, config)
    return clampVoice(value, config)
  }

  private logCall(name: string, args: Record<string, unknown>, outcome: string, ms: number): void {
    if (!this.deps.config().logToolCalls) return
...
function xiaozhiAlias(name: string): string {
...
function clampVoice(value: unknown, config: ResolvedConfig): unknown {
...
export function coveredCapabilityIds(tools: readonly DshToolDefinition[]): Set<string> {
  const ids = new Set<string>()
  for (const tool of tools) for (const id of tool.capabilities) ids.add(id)
  return ids
}
