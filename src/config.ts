...
export function resolveHomeDir(config?: Pick<Config, 'homeDir'>): string {
  ...
  if (dshHome !== '') return path.join(path.resolve(dshHome), 'dsh-xiaozhi')
  return path.join(homedir(), '.dsh', 'dsh-xiaozhi')
}

export function resolveSettingsFile(config?: Pick<Config, 'homeDir'>): string {
  return path.join(resolveHomeDir(config), 'settings.json')
}

/** Merge defaults ← row config ← persisted overrides, then normalise. */
export function resolveConfig(row: Config | undefined, overrides?: Config | undefined): ResolvedConfig {
  ...
  resolved.endpointHeaders =
    resolved.endpointHeaders && typeof resolved.endpointHeaders === 'object'
      ...
      : {}
  if (!resolved.serverPath.startsWith('/')) resolved.serverPath = `/${resolved.serverPath}`
  resolved.serverPath = resolved.serverPath.replace(/\/+$/, '') || '/mcp/xiaozhi'
  if (!resolved.apiPathPrefix.startsWith('/')) resolved.apiPathPrefix = `/${resolved.apiPathPrefix}`
...
export function readOverrides(config?: Pick<Config, 'homeDir'>): Config {
  const file = resolveSettingsFile(config)
  try {
    if (!existsSync(file)) return {}
    const parsed = JSON.parse(readFileSync(file, 'utf8'))
    if (parsed !== null && typeof parsed === 'object' && !Array.isArray(parsed)) {
      for (const key of NEVER_PERSISTED) delete (parsed as Record<string, unknown>)[key]
 ...
 */export function writeOverrides(
  config: Pick<Config, 'homeDir'>,
  patch: Config,
): { overrides: Config; resolved: ResolvedConfig } {
  ...
  for (const [key, value] of Object.entries(patch)) {
    if (value === undefined) continue
    if (NEVER_PERSISTED.includes(key)) continue
    if (key === 'endpointHeaders') {
      next.endpointHeaders = { ...(current.endpointHeaders ?? {}), ...(value as Record<string, string>) }
    } else {
...
export function clearOverrides(config: Pick<Config, 'homeDir'>): void {
  const file = resolveSettingsFile(config)
  try {
...
export function isMaskedEndpoint(url: unknown): boolean {
