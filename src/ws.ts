 ...
 */
function encodeFrame(opcode: number, payload: Buffer, mask: boolean, fin = true): Buffer {
  const len = payload.length
  let headerLength = 2
  if (len >= 65536) headerLength += 8
  else if (len >= 126) headerLength += 2
...
class FrameDecoder {
  ...
  constructor(
     ...
     */
    private readonly expectMasked: boolean | undefined = undefined,
  ) {}
  ...
  push(chunk: Buffer): void {
    ...
    try {
      ...
      this.handlers.onProtocolError(
        err instanceof WsProtocolError ? err.message : 'protocol error: malformed frame',
      )
    }
  }

  /** @returns byte count consumed, 0 when the buffer holds an incomplete frame. */
  private decodeOne(): number {
    ...
    if (rsv !== 0) throw new WsProtocolError('protocol error: reserved bits set (no extension negotiated)')
    ...
    if (payloadLen === 126) {
      ...
      if (high !== 0) throw new WsProtocolError('protocol error: frame larger than 4 GiB')
      payloadLen = low
      offset += 8
    }
    ...
    if (buf.length < offset + payloadLen) return 0
    ...
    if (opcode >= 0x8) {
      if (!fin || payloadLen > 125) throw new WsProtocolError('protocol error: malformed control frame')
      if (opcode === OP_PING) this.handlers.onPing(payload)
      else if (opcode === OP_PONG) this.handlers.onPong(payload)
    ...
    if (opcode === OP_CONT) {
      if (this.fragmentOpcode === 0) throw new WsProtocolError('protocol error: continuation without a start frame')
    ...
    if (this.fragmentOpcode !== 0) throw new WsProtocolError('protocol error: nested fragmented message')
  ...
  private deliver(opcode: number, payload: Buffer): void {
    if (opcode !== OP_TEXT) return // binary is acknowledged but ignored
    this.handlers.onText(payload.toString('utf8'))
  }
}

class SocketConnection implements WsConnection {
  closed = false
  private readonly decoder: FrameDecoder
  private readonly messageHandlers: ((text: string) => void)[] = []
  private readonly closeHandlers: ((info: WsCloseInfo) => void)[] = []
  ...
  constructor(
    ...
    this.decoder = new FrameDecoder(
      {
        ...
        onPing: payload => {
          if (!this.closed) this.socket.write(encodeFrame(OP_PONG, payload, this.maskOutbound))
        },
        onPong: () => {
    ...
    try {
      ...
      const tunable = this.socket as unknown as {
    ...
    if (head && head.length > 0) this.decoder.push(head)
  }

  get bufferedAmount(): number {
    return Number((this.socket as unknown as { writableLength?: number }).writableLength ?? 0)
  }
  ...
  private finish(code: number, reason: string): void {
    ...
    const info: WsCloseInfo = { code, reason }
...
export function acceptUpgrade(
  ...
  if (options.subprotocol) lines.push(`Sec-WebSocket-Protocol: ${options.subprotocol}`)
  socket.write(`${lines.join('\r\n')}\r\n\r\n`)

  return new SocketConnection(
    socket,
    false,
    options.maxPayloadBytes ?? DEFAULT_MAX_PAYLOAD,
    head,
  )
}

/** Open an outbound WebSocket. Resolves after a verified 101 handshake. */
export function connectWebSocket(url: string, options: WsConnectOptions = {}): Promise<WsConnection> {
  return new Promise((resolve, reject) => {
    ...
    req.on('upgrade', (res: IncomingMessage, socket: Duplex, head: Buffer) => {
      ...
      resolve(
        new SocketConnection(socket, true, options.maxPayloadBytes ?? DEFAULT_MAX_PAYLOAD, head),
      )
    })
    ...
    req.on('response', res => {
      ...
      res.on('end', () => {
        ...
        reject(
          new Error(
            `WebSocket handshake rejected: HTTP ${status}${body ? ` – ${body.replace(/\s+/g, ' ').trim()}` : ''}`,
          ),
        )
      })
