...
* id 会被缩短：先剥掉 `session-` 前缀再取前 8 位（如 `session-4ac05afc-…` → `4ac05afc`），
  避免所有会话都坍缩成同一个 `[session-]` 前缀；
* 引用会话/工作区时，**短 id、完整 id 或标题都能被接受**：能力层会在派发前把引用解析回
  完整 id（唯一匹配才放行；多义时报出候选，查无时报「找不到」），列表里返回的短 id 可以直接回传。
